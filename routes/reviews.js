...
router.post(
    "/movies/:movieId/reviews/:reviewId/:action",
    authController.isAuthenticated,
    async (req, res) => {
        const movieId = req.params.movieId;
        ...
        const [likes] = await db.query("SELECT * FROM review_likes WHERE user_id = ? AND review_id = ?", [userId, reviewId]);
        if (likes.length > 0) {
            return res.status(400).json({ message: "User has already liked or disliked the review" });
        }
        ...
        if (action === 'dislike') {
            await db.query("UPDATE reviews SET dislikes_count = dislikes_count + 1 WHERE id = ?", [reviewId]);
        }
        //save the like or dislike to the database
        try {
            const [result] = await db.query("INSERT INTO review_likes(review_id, user_id, action) VALUES (?, ?, ?)", [reviewId, userId, action]);
            if (result.affectedRows === 1) {
